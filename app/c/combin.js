...
var param = {
    ...
    urlBasedCharset: {},
    fns: [],
    hosts: require('./config').commonHostsIp
};

var cosoleResp = function (type, c) {
        console.log(type + '&' + c);
    };

var Combo = {
    init: function (urls, options, recordRequest) {
        if (recordRequest) {
            cosoleResp = recordRequest;
        }
        var userHome = process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH;//兼容windows
        var cacheDir = path.join(userHome, '.combo/cache');
        if (!fs.existsSync(cacheDir)) {
            mkdirp(cacheDir);
        }
//    var userConfigPath = path.join(userHome, '.staticS/config.json');
//    if(!fs.existsSync(userConfigPath)){
//        if(!fs.existsSync(path.join(userHome,'.staticS'))){
//            mkdirp.sync(path.join(userHome,'.staticS'));
//        }
...
//    }
        ...
        return function (req, res, next) {
            ...
            if (prefix === -1) {
                ...
                http.get(requestOption,function (resp) {
                    ...
                    if (resp.statusCode !== 200) {
                        ...
                        cosoleResp('Not found', requestOption.host + requestOption.path + ' (host:' + headerHost + reset + ')');
                        res.writeHead(404);
            ...
            for (var i = 0, len = files.length; i < len; i++) {
                ...
                //combo URL有时候会多一个逗号
                if (file === "") continue;
                var fullPath = prefix + filterUrl(file);
                if (i === 0) {
                ...
                (function (id) {
                    ...
                    http.get(requestOption,function (resp) {
                        if (resp.statusCode !== 200) {
                            ...
                            cosoleResp('Not found', requestOption.host + reqPath + reqArray[id].file + '(' + 'host:' + headerHost + ')');
...
function cacheFile(fullPath, content, encode) {
